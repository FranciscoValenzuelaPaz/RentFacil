<style>
    <?php include("../../../CSS/registrarMaquinariaFormulario.css"); ?><?php include("../../../Header-Footer/header5.php"); ?>
        ...
        $query = $dbh->prepare("UPDATE tabla_maquinarias SET titulo = ?, descripcion = ?, tipo = ?, bencina = ?, ubicacion = ?, id_comuna = ?,
     montoArriendo = ? WHERE id_maquinaria = ?;");
        $resultado = $query->execute([$titulo, $descripcion, $tipo, $bencina, $ubicacion, $id_comuna, $montoArriendo, $id_maquinaria]); # Pasar en el mismo orden de los ?
        if ($resultado === TRUE) {
            $mensaje = "editado";
            echo '
             <script>
